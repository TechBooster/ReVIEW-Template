...
#@# 難しくなってしまいます → 難しくなってしまっていました。

そこで、出てきたのがフロントエンドフレームワークです。近年だと AngularJS/React/Vue.jsが有名です。
#@# フロントエンドフレームワークは、~~によって先の問題を解決しました。的なのが欲しい
...
#@# 箇条書きだけど敬語ってのが違和感あるので、
#@# * TypeScript/ES6ベース
#@# 書き方がTypeScipt/ES6のクラスベースになっています。~~~
#@# とかだと違和感ない
...
#@# グラフをコンポーネント化するには → グラフのコンポーネント化には
...
#@# 簡単に作成！、といった → 簡単に作成！といった （、いらないかも）

今回のコンポーネントだけ、例外的にTypeScriptではなくJavaScriptで書いています。@<fn>{why_hs}
#@# 今回の → なお、今回の (なお的なのが欲しい)
...
Stateは、先ほど、説明したディレクトリの@<tt>{store}の配下にファイルを作ることで
#@# Stateは、先ほど、説明したディレクトリ → Stateは、先ほど説明したディレクトリ （、がいらない）
#@# の配下にファイルを作ることで → の配下にファイルを作ると
...
Stateはさきほども言ったとおり、アプリ中すべてのコンポーネントから使われるデータなので
#@# Stateはさきほども言ったとおり、 → Stateはさきほども書いたとおり、
いろいろなデータをもつ必要があります。
...
#@# TypeScriptinterfaceで事前に定義しておくことができます。→ TypeScriptinterfaceを使うと事前に定義しておくことができます。
...
#@# 今まで説明してきたVuexの関数を実際にコンポーネントに → 今まで説明してきたVuexの関数ですが、これを実際にコンポーネントに組み込んでみましょう。 （なんかワンクッションほしい）
...
アプリの形ができ上がっていきます。
#@# アプリの形ができ上がっていきます。 → アプリのフロントエンド部分が完成しました。（章末なので、完成感欲しい）
