...
今回のサンプルアプリケーションはすべて以下のGithubレポジトリにあります。
本文の説明と合わせて参照いただけると理解が進むと思います。

== 今回作るアプリ
今回は、以下の様な構成でアプリを実装していきます。
#@# アプリの構成図を入れる
#@# アプリの構成の説明
...
Node.jsの動作環境のインストールは今回は省略いたしますので、適宜、Web等で調査いただき
導入いただければと思います。
...
他にもvue-cliなどNuxtのプロジェクトを作成する方法がありますが、Nuxtでプロジェクトを作る場合は、2019年現在 create-nuxt-appを利用するのが非常におすすめです。