...
=== 棋譜データ(CSA形式)の読み込み
棋譜データにはいくつか種類があります、今回はCSA形式で扱います。将棋DB2@<fn>{endofn02}など公開されている棋譜データを扱いましょう。読み込みライブラリにはgunyarakun/python-shogi@<fn>{endofn03}を使用します。
...
データさえあればそれをMIDIに変換できます。変換時のルールが音楽を決めるため重要な部分となりますが、今回は盤面の中央がC4の音になるように変換することを考えます。
...
MIDIは動画と一緒に扱うには扱いづらいためwaveに変換しておきます。pythonでも可能です。ここでは楽器選びなども行うためGarageBandを使いました。


=== Pythonとffmpegで動画向け連続画像出力
今回はスクリプトから動画生成する方法の中でも難易度が低いパラパラマンガ方式で動画を作成します。Pythonで動画にする連続画像を出力し後でパラパラマンガのように結合することで動画になるようにします。
...
まず連続する画像から動画を作成します。1秒あたりの枚数に注意して出力します。ffmpeg@<fn>{endofn08}により以下のように書けます。BPMからフレームレートを決めましょう。
...
ある瞬間の寄付について、3,5,7行目だけに着目し、1-9列について順番に見て駒があるなら音を鳴らし、駒がないなら音を出さないというルールで変換しています。
